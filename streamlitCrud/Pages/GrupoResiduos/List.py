...
def List():
    ...
    if params.get("id") == None:
        ...
        for item in GrupoResiduosController.SelecionarTodos():
            ...
            col1.write(item.id_residual_group)
            col2.write(item.description)
            button_space_excluir = col3.empty()
            on_click_excluir = button_space_excluir.button(
                'Excluir', 'btnExcluir' + str(item.id_residual_group))
            button_space_alterar = col4.empty()
            on_click_alterar = button_space_alterar.button(
                'Alterar', 'btnAlterar' + str(item.id_residual_group))

            if on_click_excluir:
                GrupoResiduosController.Excluir(item.id_residual_group)
                button_space_excluir.button(
                    'Excluído', 'excluded' + str(item.id_residual_group))
                st.experimental_rerun()
            if on_click_alterar:
                st.experimental_set_query_params(
                    id=[item.id_residual_group]
                )
                st.experimental_rerun()
