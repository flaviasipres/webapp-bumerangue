...
def List():
    ...
    if params.get("id") == None:
        ...
        for item in TipoResiduosController.SelecionarTodos():
            col1, col2, col3, col4 = st.columns([1,1,1,1])
            col1.write(item.id_residual_type)
            col2.write(item.description)
            button_space_excluir = col3.empty()
            on_click_excluir = button_space_excluir.button(
                'Excluir', 'btnExcluir' + str(item.id_residual_type))
            button_space_alterar = col4.empty()
            on_click_alterar = button_space_alterar.button(
                'Alterar', 'btnAlterar' + str(item.id_residual_type))

            if on_click_excluir:
                TipoResiduosController.Excluir(item.id_residual_type)
                button_space_excluir.button(
                    'Excluído', 'excluded' + str(item.id_residual_type))
                st.experimental_rerun()
            if on_click_alterar:
                st.experimental_set_query_params(
                    id=[item.id_residual_type]
                )
                st.experimental_rerun()
